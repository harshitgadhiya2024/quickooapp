...
class EmailOtpScreen extends StatefulWidget {
  ...
  EmailOtpScreen({super.key, required this.email, required this.sendOtp});
...
class _EmailOtpScreenState extends State<EmailOtpScreen> {
  ...
  void showOtpInfo(String otp){
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("OTP send to ${widget.email}: $otp")));
  }
  ...
  void verifyOtp() {
    ...
    if (enteredOtp == currentOtp) {
      signupController.saveEmail(widget.email);
      setState(() {
        isLoading = false;
      });
      showDialog(
  ...
  void ResendOtp() async {
    ...
    var result = await emailVerificationController.SendEmailOtp(email:widget.email, otp: otp);
